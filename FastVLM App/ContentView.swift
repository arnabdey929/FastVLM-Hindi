...
struct ContentView: View {
    ...
    @State private var translatorModel = TranslatorModel()
    ...
    func analyzeVideoFrames(_ frames: AsyncStream<CVImageBuffer>) async {
        for await frame in frames {
            ...
            let generatedText = await MainActor.run { model.output }
            let translatedText = translatorModel.autoRegression(inputText: generatedText)
            
            await MainActor.run { model.output = translatedText }
